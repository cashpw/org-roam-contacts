...
(defcustom org-roam-contacts--address-property
  "CONTACT_ADDRESSES"
...
(defcustom org-roam-contacts--phone-property
  "CONTACT_PHONES"
...
(defcustom org-roam-contacts--reminders-heading
  "Reminders"
  "Heading text for reminders."
  :type 'string
  :group 'org-roam-contacts)

(defcustom org-roam-contacts--birthday-reminder-heading
  "%s's birthday"
...
(defcustom org-roam-contacts--directory
  org-roam-directory
  "Directory path."
  :type 'string
  :group 'org-roam-contacts)
...
(cl-letf (((symbol-function 'current-time) (lambda ()
  ...
  (cl-assert
   (equal
    (org-roam-contacts--get-next-annual-time (date-to-time "2022-10-10T08:00:00-0700"))
    (date-to-time "2022-10-10T08:00:00-0700"))
   "Next time should be this year (2022) because the date hasn't yet passed.")
  (cl-assert
   (equal
    (org-roam-contacts--get-next-annual-time (date-to-time "2022-10-01T08:00:00-0700"))
    (date-to-time "2023-10-01T08:00:00-0700"))
   "Next time should be next year (2023) because the date has passed.")
  (cl-assert
   (equal
    (org-roam-contacts--get-next-annual-time (date-to-time "2000-10-10T08:00:00-0700"))
    (date-to-time "2023-10-10T08:00:00-0700"))
   "Next time should be next year (2023) because the date has passed.")
  (cl-assert
   (equal
    (org-roam-contacts--get-next-annual-time (date-to-time "2000-10-01T08:00:00-0700"))
    (date-to-time "2023-10-01T08:00:00-0700"))
   "Next time should be next year (2023) because the date has passed."))
...
(defun org-roam-contacts--top-level-heading-exists-p (heading-text)
  ...
  (member
   heading-text
   (org-roam-contacts--list-top-level-headings)))

(defun org-roam-contacts--insert-top-level-heading-if-absent (heading-text &optional pos)
...
Returns nil if the heading already existed."
  (let ((pos
         (or pos
             (point-max))))
    (unless (org-roam-contacts--top-level-heading-exists-p
             heading-text)
      (goto-char pos)
...
(defun org-roam-contacts-insert-reminder (reminder-text &optional time repeater-interval)
  "Insert a reminder with REMINDER-TEXT heading at TIME with REPEATER-INTERVAL."
  (interactive "sReminder heading: ")
  (let ((time-string
         (format-time-string
          (if repeater-interval
              (format
               "<%%F %s>"
               repeater-interval)
            "<%F>")
    ...
    (org-roam-contacts--goto-heading
     org-roam-contacts--reminders-heading)
...
(defun org-roam-contacts--get-birthday-time ()
  ...
  (org-time-string-to-time
   (org-roam-contacts--get-property
    org-roam-contacts--birthday-property)))
...
2. Annually ADVANCE-NOTICE-DAYS before the person's birthday"
  ...
  (when (and
         (org-roam-contacts-file-p)
         (org-roam-contacts--has-property-p
          org-roam-contacts--birthday-property))
    (let* ((birth-time
            (org-roam-contacts--get-birthday-time))
           (contact-name
            (org-roam-contacts--get-name))
           (birthday-heading-text
            (format
             org-roam-contacts--birthday-reminder-heading
             contact-name))
           (upcoming-birthday-heading-text
            (format
             org-roam-contacts--birthday-advanced-reminder-heading
             contact-name
             advance-notice-days)))
        ...
        (let* ((reminder-time
                (org-roam-contacts--get-next-annual-time
                 (time-subtract
                  ...
                  (days-to-time
                   advance-notice-days)))))
          (org-roam-contacts-insert-reminder
           upcoming-birthday-heading-text
           reminder-time
           "++1y")))
      (unless (org-find-exact-headline-in-buffer
